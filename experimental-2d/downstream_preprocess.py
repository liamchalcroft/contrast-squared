...
def get_data_chunks(h5_path, task='denoising', split='train', train_ratio=0.6, val_ratio=0.2, modality='t1', site='GST'):
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        print(f"Sample modality: {task_group[subjects[0]][f"image_{subjects[0]}" if task == "segmentation" else modality][modality].attrs.get('modality')}")
        print(f"Sample site: {task_group[subjects[0]][f"image_{subjects[0]}" if task == "segmentation" else modality][modality].attrs.get('site')}")
        print(f"Sample data shape: {task_group[subjects[0]][f"image_{subjects[0]}" if task == "segmentation" else modality][modality][:].shape}")
