...
class H5SliceDataset(Dataset):
        ...
        with h5py.File(h5_path, 'r', libver='latest', swmr=True) as h5_file:
            ...
            for subject in h5_file.keys():
                if 'contrasts' in h5_file[subject].keys():  # qMRI data
                    ...
                    self.data_chunks[subject] = data[:, None, :, :]  # [slices, 1, H, W]
    ...
    def __getitem__(self, idx):
        ...
        if len(data.shape) == 4 and data.shape[1] > 1:  # qMRI data
            ...
            slice_data = data[slice_idx]  # Already has channel dimension [1, H, W]
            images = {f"image{i+1}": slice_data for i in range(self.num_views)}
        
        # Convert to tensor (no need to add channel dimension as it's already there)
        images = {k: torch.from_numpy(v) for k, v in images.items()}
