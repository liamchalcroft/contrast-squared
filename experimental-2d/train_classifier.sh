...
for MODEL_NAME in "${!MODELS[@]}"; do
    ...
    for MODALITY in "${MODALITIES[@]}"; do
        for SITE in "${SITES[@]}"; do
            echo "Training classifier: $MODEL_NAME ($MODEL_ARGS) on $MODALITY data from $SITE"
            ...
            python train_classifier.py \
                ...
                --learning_rate $LR \
                --amp \
                2>&1 | tee "$CHECKPOINT_DIR/classifier_${MODALITY}_${SITE}.log"
            
            # Check if training completed successfully
            if [ $? -eq 0 ]; then
                echo "Successfully completed classifier training for $MODEL_NAME on $MODALITY data from $SITE"
            else
                echo "Error training classifier for $MODEL_NAME on $MODALITY data from $SITE"
            fi
        done
    done
done 