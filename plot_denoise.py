...
def spider_plot(results_df, metric="MSE", class_name=None):
...
for percentage in training_percentages:
    ...
    for class_name in [None] + list(classes):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(percentage_dir, f"mse_by_modality_dataset{class_suffix}.png"), 
                    dpi=300, bbox_inches='tight')
        plt.close()

        # 2. Boxplot of PSNR scores by model and dataset
        plt.figure(figsize=(15, 8))
        ...
        plt.legend(title="Method", title_fontsize=12, fontsize=11, bbox_to_anchor=(1.05, 1))
        plt.tight_layout()
        plt.savefig(os.path.join(percentage_dir, f"psnr_by_modality_dataset{class_suffix}.png"), 
                    dpi=300, bbox_inches='tight')
        plt.close()

        # 3. Spider plot of MSE scores by modality and dataset
        plt.figure(figsize=(10, 10))
        spider_plot(results_df, metric="MSE", class_name=class_name)
        plt.savefig(os.path.join(percentage_dir, f"mse_spider_plot{class_suffix}.png"))
        plt.close()

        # 4. Spider plot of PSNR scores by modality and dataset
        plt.figure(figsize=(10, 10))
        spider_plot(results_df, metric="PSNR", class_name=class_name)
