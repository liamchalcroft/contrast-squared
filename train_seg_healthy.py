...
def get_loaders(
  ...
  if modality == "t1":
      ...
      data_list.sort()
      data_dict = [{"image": f, "file": f, "label": [f.replace("p_IXI", "c1p_IXI"), f.replace("p_IXI", "c2p_IXI"), f.replace("p_IXI", "c3p_IXI")], "dataset": "IXI", "site": "guys", "modality": "T1w", "IXI_ID": int(os.path.basename(f).split("-")[0][5:])} for f in data_list]
  elif modality == "t2":
      data_list = glob.glob("/home/lchalcroft/Data/IXI/guys/t2/preprocessed/p_IXI*-T2.nii.gz")
      data_list.sort()
      data_dict = [{"image": f, "file": f, "label": [f.replace("p_IXI", "c1p_IXI"), f.replace("p_IXI", "c2p_IXI"), f.replace("p_IXI", "c3p_IXI")], "dataset": "IXI", "site": "guys", "modality": "T2w", "IXI_ID": int(os.path.basename(f).split("-")[0][5:])} for f in data_list]
  elif modality == "pd":
      data_list = glob.glob("/home/lchalcroft/Data/IXI/guys/pd/preprocessed/p_IXI*-PD.nii.gz")
      data_list.sort()
      data_dict = [{"image": f, "file": f, "label": [f.replace("p_IXI", "c1p_IXI"), f.replace("p_IXI", "c2p_IXI"), f.replace("p_IXI", "c3p_IXI")], "dataset": "IXI", "site": "guys", "modality": "PDw", "IXI_ID": int(os.path.basename(f).split("-")[0][5:])} for f in data_list]
            ...
            with torch.no_grad():
                ...
                for i, batch in enumerate(val_loader):
                    ...
                    if i < 16:
                        ...
                        seg_list.append(seg.argmax(dim=1, keepdim=True).float()[0,...,seg.shape[-1]//2])
